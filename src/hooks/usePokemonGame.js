...
const initializePlayerData = useCallback(() => {
    ...
    try {
    const saved = JSON.parse(localStorage.getItem('pokemonGameData') || '{}')
...
}, [missions, addExp, savePlayerData, showNotification])
...
const encounterWildPokemon = useCallback(async () => {
    ...
    const newData = {
        ...playerData,
        totalEncounters: playerData.totalEncounters + 1
    }
...
}, [playerData, savePlayerData, checkMissions, showNotification])
...
const attemptCapture = useCallback(async () => {
    ...
    if (success) {
        const alreadyCaught = playerData.collection.some(p => p.id === currentPokemon.id)
        
        const newCollection = alreadyCaught 
        ? playerData.collection
        : [...playerData.collection, { ...currentPokemon, caughtAt: new Date().toISOString() }]
        
        let newData = {
        ...playerData,
        pokemonCaught: playerData.pokemonCaught + 1,
...
}, [currentPokemon, playerData, isCapturing, addExp, savePlayerData, checkMissions, showNotification, encounterWildPokemon])
...
useEffect(() => {
    ...
    // Verificar misiones iniciales
    setTimeout(() => checkMissions(initialData), 100)
}, [initializePlayerData, initializeMissions, checkMissions])
